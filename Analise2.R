...
setwd("C:/Users/dases/Desktop/Nova pasta/Trabalho_ECDB") #Path_to_working_directory

rawcount<-read.table ("datasets/data_mrna_seq_v2_rsem.txt",header=TRUE,  sep="\t",  row.names=1)
...
anno <- read.table ("datasets/data_clinical_sample.txt",header=TRUE,  sep="\t", row.names = 1, skip=4) # Ajuste o 'skip' conforme necessário para ignorar as linhas de cabeçalho
...
#anno <- anno %>% 
#  as.data.frame %>%
#  filter(anno$CANCER_TYPE =='Condition_A' |anno$CANCER_TYPE =='Condition_B' | anno$CANCER_TYPE == 'Condition_C' )  %>%
#  arrange(CANCER_TYPE)  	#Arrange rows by padj values 
...
anno <- anno %>% 
  ...
  arrange(SAMPLE_TYPE) # Ajuste conforme necessário
...
############## SVA #####################
  ...
  adjusted_counts <- ComBat_seq(rawcount, batch=anno$Batch, group=anno$CANCER_TYPE) ##In ComBat-seq, user may specify biological covariates, whose signals will be preserved in the adjusted data. I
...
plotPCA(vst, intgroup="CANCER_TYPE")  ### Plot PCA 
...
ggplot(top20_norm) +
  geom_point(aes(x = gene, y = normalized_counts, color = CANCER_TYPE)) +

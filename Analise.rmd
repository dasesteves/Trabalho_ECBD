...
dados_sample <- read.delim("x/x/x/data_clinical_sample.txt", header = TRUE, stringsAsFactors = FALSE)
dados_patient <- read.delim("x/x/x/data_clinical_patient.txt", header = TRUE, stringsAsFactors = FALSE)
...
describe(dados_sample_limpo)

#Nao sei se e preciso fazer isto
#missing_values <- sapply(dados_sample_limpo, function(x) sum(is.na(x)))
#print(missing_values[missing_values > 0])
#dados_sample_limpo$Neoplasm.Histologic.Grade[is.na(dados_sample_limpo$Neoplasm.Histologic.Grade)] <- 0
#dados_sample_limpo$Tissue.Prospective.Collection.Indicator[is.na(dados_sample_limpo$Tissue.Prospective.Collection.Indicator)] <- 0
#dados_sample_limpo$Tissue.Retrospective.Collection.Indicator[is.na(dados_sample_limpo$Tissue.Retrospective.Collection.Indicator)] <- 0

head(dados_patient_limpo)
str(dados_patient_limpo)
summary(dados_patient_limpo)
describe(dados_patient_limpo)

#Eu fiz vários codigos que comparam varios fatores mas não sei se mete-se os codigos com os gráficos ou se mete-se só os gráficos e aonde por estas informações
...
ggplot(dados_patient_limpo, aes(x = TCGA.PanCanAtlas.Cancer.Type.Acronym, y = Diagnosis.Age)) +
  geom_boxplot() +
  labs(title = "Distribuição da Idade de Diagnóstico pelo câncro", x = "Câncro", y = "Idade de Diagnóstico")

ggplot(dados_patient_limpo, aes(x = Overall.Survival..Months., y = Months.of.disease.specific.survival)) +
  geom_point() +
  labs(title = "Sobrevivência Geral vs. Sobrevivência Específica da Doença",
       x = "Sobrevivência Geral (Meses)",
       y = "Sobrevivência Específica da Doença (Meses)")
...
ggplot(dados_patient_limpo, aes(x = Radiation.Therapy, y = Overall.Survival..Months., fill = Radiation.Therapy)) +
  geom_boxplot() +
  labs(title = "Sobrevivência Geral com e sem Terapia de Radiação",
       x = "Terapia de Radiação",
       y = "Sobrevivência Geral (Meses)",
       fill = "Terapia de Radiação")

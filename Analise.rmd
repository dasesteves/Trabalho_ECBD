...
ggplot(dados_sample_limpo, aes(x = MSI.MANTIS.Score, y = TMB..nonsynonymous.)) +
  ...
  scale_x_continuous(limits = c(0.24, 0.38)) +
...
#Cria um gráfico de barras da distribuição do status de sobrevivência pela raça
...
#Criar uma tabela de contingência das duas variáveis
tabela_contingencia1<- table(dados_patient_limpo$Race.Category, dados_patient_limpo$Overall.Survival.Status)

#Realizar o teste qui-quadrado de independência
resultado_teste1 <- chisq.test(tabela_contingencia1)

#Exibir o resultado do teste
print(resultado_teste)


#Cria um gráfico de barras da distribuição do status de doença livre pela raça
...
# Criar uma tabela de contingência das duas variáveis
tabela_contingencia2 <- table(dados_patient_limpo$Race.Category, dados_patient_limpo$Disease.Free.Status)

# Realizar o teste qui-quadrado de independência
resultado_teste2 <- chisq.test(tabela_contingencia2)

# Exibir o resultado do teste
print(resultado_teste2)


#Cria um gráfico de dispersão relacionando a idade do diagnóstico com a sobrevivência geral
...
#Calcular o coeficiente de correlação de Pearson
correlation2 <- cor(dados_patient_limpo$Diagnosis.Age, dados_patient_limpo$Overall.Survival..Months.)
...
#Calcular a correlação entre a idade do diagnóstico e a sobrevivência geral
correlation_test3 <- cor.test(dados_patient_limpo$Diagnosis.Age, dados_patient_limpo$Overall.Survival..Months.)
...
# Carregar os dados
data <- dados_patient_limpo

# Ajustar o modelo de regressão linear
modelo <- lm(Diagnosis.Age ~ Overall.Survival..Months., data)

# Criar o gráfico de dispersão com a linha de regressão
plot(data$Overall.Survival..Months., data$Diagnosis.Age, 
     main = "Gráfico de Regressão", xlab = "Sobrevivência Global (Meses)", ylab = "Idade do Diagnóstico")
